 ...
 *
 * ## Design Principles
 * - **Polymorphic by media type**: Each "Ready" state is specific to a media type
 *   (e.g., [ImageReady], [PdfReady]), enabling the UI layer to render type-specific
 *   Composables via `when` branches that satisfy Open/Closed Principle.
 * - **File-path only, never ByteArray**: All states hold only a decrypted file path,
 *   letting Glide (or a PDF renderer) handle downsampling and memory management.
 *   This is the critical OOM safety boundary for attachments up to 100MB+.
 * - **Parcelable but ephemeral**: The file path is [IgnoredOnParcel] because
 *   decrypted files are ephemeral and must not survive process death.
 *
 * ## State Machine
 * ```
